...
| 9 | CC-009 | Workers・D1実績連携 | P1 | 一部完了 | Workers requests、D1 reads/writes/storageはDashboardと一致した。CPUは`scriptName`単位の期間集計へ修正し、サービス別P50/P99表示を本番反映した。再取得時のrequests 404・268・178はDashboardと一致し、Cloud CostのP50/P99も0.798/8.523 ms対0.81/8.84 msで一致。HandoffとInstallerはadaptive集計差が残り、Dashboardが7日超のCustom期間をNo dataとするため、同一UTC期間での最終再照合だけを残す。 |
